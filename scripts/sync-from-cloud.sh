...
# - 5/13 新增：检测源 stale（intraday 时段 snapshot_at 年龄超阈值），下推 PushDeer
#   阈值由 BACKUP_INTRADAY_INTERVAL_MIN 推出，跟云端 rquant-backup.timer 对齐（#243）
...
# 云端盘中备份的触发间隔。**唯一来源**：deploy/systemd/rquant-backup.timer 的
# `OnCalendar=Mon..Fri *-*-* 9..15:0/15`（#243 / 协调者 2026-09-08 定为 15 分钟）。
# 改那个文件时必须同步改这一个数字，下面所有判定和告警文案都从它推出来。
BACKUP_INTRADAY_INTERVAL_MIN=15
# 单次快照实测 8m16s–8m50s（#243），向上取 10 分钟；再容忍漏掉一个触发周期，
# 所以 snapshot_at 的年龄超过 2×间隔 + 10 分钟才算真的停了。
# 本地 sync 每 5 分钟跑一次，比云端快，「这一轮跟上一轮一样」是常态，不是异常。
BACKUP_MAX_SNAPSHOT_AGE_MIN=$(( 2 * BACKUP_INTRADAY_INTERVAL_MIN + 10 ))

# ISO8601 UTC → epoch。先 BSD date（mac 本机），再 GNU date（Linux 上跑同一脚本时）。
snapshot_epoch() {
...
if [[ -z "${new_snapshot}" ]]; then
    ...
    if [[ -z "${last_snapshot}" ]]; then
        ...
        # intraday 时段下 snapshot_at 跟上次一样：本地 5 分钟一轮、云端 15 分钟一轮，
        # 一致是常态。只有 snapshot_at 的**年龄**超过 BACKUP_MAX_SNAPSHOT_AGE_MIN
        # 才说明 backup.timer 真的停了（v0.11.3 风险）。
        now_ts=$(date +%s)
        snap_ts=$(snapshot_epoch "${new_snapshot}")
        if [[ -z "${snap_ts}" ]]; then
            log "${sync_label}: ${size} (snapshot_at=${new_snapshot} 无法解析时间，跳过 stale 判定)"
            snapshot_age_min=""
        else
            snapshot_age_min=$(( (now_ts - snap_ts) / 60 ))
        fi
        should_alert=0
        if [[ -n "${snapshot_age_min}" ]] \
            && (( snapshot_age_min <= BACKUP_MAX_SNAPSHOT_AGE_MIN )); then
            log "${sync_label}: ${size} (snapshot_at=${new_snapshot} 已 ${snapshot_age_min}min，" \
                "云端 ${BACKUP_INTRADAY_INTERVAL_MIN}min 一轮，未超 ${BACKUP_MAX_SNAPSHOT_AGE_MIN}min，正常)"
        elif [[ -n "${snapshot_age_min}" ]]; then
            log "WARN: source stale (snapshot_at=${new_snapshot} 已 ${snapshot_age_min}min >" \
                "${BACKUP_MAX_SNAPSHOT_AGE_MIN}min); intraday backup 可能已停"
            should_alert=1
        ...
        if (( should_alert == 1 )); then
            ...
            body="本地 sync 拉到的 snapshot_at 已经 ${snapshot_age_min} 分钟没动（${new_snapshot}）。
盘中云端 backup 每 ${BACKUP_INTRADAY_INTERVAL_MIN}min 触发一次、单次约 9 分钟，
所以正常年龄不会超过 ${BACKUP_MAX_SNAPSHOT_AGE_MIN} 分钟。
可能 rquant-backup.timer 没在调度（v0.11.3 风险再发），或每轮都在超时被杀。
...
2. NEXT 应在 ${BACKUP_INTRADAY_INTERVAL_MIN}min 内（不是 17:30）
