...
3. 新旧实时链路完成真实交易日 shadow 并行，达到 §18.1 的量化退休门后才能移除旧的
...
- 发布 current 快照和不可变 spool；保留旧路径影子运行比较，天数按 §18.1 退休门。
...
- 旧 monitor 内嵌采集/通知路径只在满足 §18.1 退休门（切流前后各 ≥ 10 个有效交易日、合计
  ≥ 20 日，且样本量、无未解释严重偏差、回滚演练三条同时成立）后删除。
...
| Phase 6 | 故障演练通过；旧路径有明确回滚版本和删除清单；满足 §18.1 退休门 |

### 18.1 退休门（冻结）

Amended per Codex round-2 order 2026-08-25, ruling 6。本节是旧实时链路能否删除的**唯一**
量化判据；文中其他地方出现的影子并行天数一律以本节为准。「有效交易日」指该日新旧两条链路
都完整运行、没有因部署/故障/停机造成整段缺口的交易日；缺口日不计入窗口，窗口不因缺口重置。

1. **切流前**：新链路以 shadow 模式与旧链路并行运行**至少 10 个有效交易日**，逐事件对账。
2. **切流后**：切流完成后再观察**至少 10 个有效交易日**。两段合计**最低 20 个有效交易日**。
3. **样本量**：shadow 窗口内累计**≥ 200 条对账通过的路由信号事件**，且**每个策略 ≥ 20 条**。
   样本不足时窗口继续延长，不得以「天数已到」替代样本量。
4. **无未解释的严重偏差**。「严重偏差」冻结为两类：
   - 路由信号的**身份或 sequence 不一致**——零容忍，出现一次即判定未通过；
   - 通知或模拟盘的**对账差异 > 0.5%**（按窗口内条数占比计）。
   低于阈值的差异必须逐条有书面解释，未解释的差异按严重偏差处理。
5. **回滚演练**：Phase 6 演练清单内的**回滚演练至少完整执行 1 次**并留下证据。

> **数字来源标注**：Codex 第二轮工单只冻结了「10 + 10 = 20 个有效交易日」这一项。第 3 条的
> 样本量（200 / 20）、第 4 条的严重偏差阈值（身份/sequence 零容忍、对账差异 0.5%）与第 5 条
> 的回滚演练次数（≥ 1）是 **Claude 提案，待 Codex 确认**；确认前不得据此宣布退休门已达标。
