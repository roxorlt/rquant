...
## 2026-07-30 增补：实时盘口与订单流数据实测

因子实验平台讨论（见 `docs/analysis/2026-07-30-factor-lab-alignment.md`）中对订单流相关数据源做了当日实测与调研：

| 数据源 | 实测/调研结论 |
|---|---|
| Tushare `rt_min` / `rt_min_daily`（分钟 RT 包） | ✅ 主 token 权限有效（已付费，续费评估进行中） |
| Tushare `rt_k`（L1 盘口快照，doc_id 372） | ❌ **两个 token 均无权限，未开通**。此前 VP 规格书按 ¥200/月 推荐用于封单监控，需单独付费开通后才能做刷新频率等三项盘中实测 |
| iTick WebSocket | 3 秒级快照聚合流（**非逐笔**），免费层限 3 只订阅；延迟 P50 1.8s / P99 3.2s，成交量完整可用于聚合/footprint（2026-07-29 一致性报告） |
| A 股 L2 逐笔（成交+委托）外部 API | 调研结论见 `docs/analysis/2026-07-30-factor-lab-alignment.md` 第四节 |

订单流因子加工的分级策略：Tier 1 分钟线近似 VP（全市场，零成本，已就绪）→ Tier 2 iTick 快照级足迹（小池）/ `rt_k` L1 全市场（待付费决策）→ Tier 3 真 L2 逐笔（待调研结论，超出现有 README 边界，引入需显式边界修订）。若真逐笔不可得，用分钟线或更小时间窗数据做「按形态不等权分摊」的近似 VP。
