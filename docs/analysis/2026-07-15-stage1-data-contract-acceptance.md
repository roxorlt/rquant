...
`v0.22.1` 将完整性分母限定为沪深 A 股代码前缀；日终 ingest 在同一事务内生成因果日指标，
历史重跑先失效后续陈旧指标；`daily-indicator-backfill` 默认只读预演，显式 `--apply`
才在安全窗口原子重建。部署后仍需先备份，再依次补齐日指标、`2025-09-25` 起历史状态和上述
