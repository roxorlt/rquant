...
>
> 2026-07-17 首次生产初始化：`v0.22.0` 已部署并应用 migration v10；三策略 manifest
> 均按 fail-closed 返回 0/72 完整日期。只读根因核验确认 N 字所需 121 日历史状态不足，
> 科创/创业策略依赖的 `daily_indicator` 未进入日终持续生成链，竞价研究湖另缺 7 个交易日
> 分区；完整性分母还误把深圳/上海 B 股计入 A 股范围。`v0.22.1` 候选修复严格 A 股分母、
> 日指标持续生成和受控补算；生产历史状态、指标与竞价分区仍须在部署后按计划回补并重新验收。
...
## 8. 2026-07-17 首次生产 manifest 根因与修复范围

`v0.22.0` 首次生产 manifest 保留为 fail-closed 证据：`n_shape`、`growth_board_surge`、
`auction_gap` 在 `2026-04-01..2026-07-16` 均为 72 个预期交易日、0 个完整日期，没有执行
分钟下载或发布 dataset snapshot。根因不是区间内“没有候选”，而是资格输入面板不完整：

1. N 字 121 交易日面板最早需要 `2025-09-25` 的历史状态，生产状态当时仅从
   `2026-04-01` 开始；
2. 日终 ingest 长期没有生成 `daily_indicator`，科创/创业策略的前一交易日 MA5/10/20/60
   依赖因此缺失；
3. 竞价研究湖在目标区间缺
   `2026-04-20`、`2026-07-07..2026-07-10`、`2026-07-13..2026-07-14`
   共 7 个交易日分区；
4. 股票并集分母按任意 `.SH/.SZ` 代码统计，混入 `200xxx.SZ` 与 `900xxx.SH` B 股。

`v0.22.1` 将完整性分母限定为沪深 A 股代码前缀；日终 ingest 从权威主库只读快照分批生成
因果日指标，关闭读连接后再在日终事务内原子写入，历史重跑先失效后续陈旧指标；
`daily-indicator-backfill` 默认只读预演，显式 `--apply` 才采用“隔离副本计算 + 二次窗口
检查 + 主库源事实指纹核对 + 短写事务”重建，长计算不持有生产主库锁。部署后仍需先备份，
再依次补齐日指标、`2025-09-25` 起历史状态和上述
7 个竞价分区，重建三份 manifest；只有覆盖门、binding 校验和固定回放都通过，才能签署
Stage 1。
