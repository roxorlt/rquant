<!-- source: https://docs.itick.org/zh-cn/rest-api/stocks/stock-klines.md -->
<!-- fetched: 2026-07-28 | locale: zh-cn | mode: md | slug: rest-api/stocks/stock-klines -->

---
title: 批量历史K线查询
description: 批量获取多只股票的完整K线数据，覆盖A股、美股、港股等全球市场数千只股票。提供标准化的时间序列数据，包含开盘价、最高价、最低价、收盘价及成交量等OHLCV字段，支持从分钟线到月线的多周期查询。
keywords: 股票批量K线API, 多股K线数据, 批量OHLC数据, 历史行情批量获取, 股票K线批量下载, 组合K线对比, 高效数据查询, 股票矩阵K线
---
...
const options = {
  ...
  path: '/stock/klines?region=HK&codes=700,9988&kType=2&limit=5',
  headers: {
    'accept': 'application/json',
    'token': '{token}'
  }
...
```bash
curl --location 'https://api.itick.org/stock/klines?region=HK&codes=700,9988&kType=2&limit=5' \
...
klines = client.get_stock_klines("HK", "700,9988", 2, 5)
print(klines)
```
...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            var klines = client.getStockKlines("HK", "700,9988", 2, 5, null);
...
func main() {
	...
	klines, err := client.GetStockKlines("HK", "700,9988", 2, 5, nil)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("Stock Klines: %+v\n", klines)
}
```
...
通过 MCP Server 调用股票批量历史K线 API：

```python
# 使用 MCP 工具 stockKlines
# 对应 REST API: GET /stock/klines
```
