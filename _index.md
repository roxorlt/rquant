...
|------|---------|------|--------|
| [[README]] | 项目总览：定位、六层架构图、7 周 MVP 路径、技术选型、下一步 | #quant #a-shares #personal-tool | week-3a-done |
| [[CLAUDE]] | Claude 项目指令：Mac 约束、技术栈、MVP 路径纪律、版本控制与部署、边界守则 | #meta #claude | active |
| [[CHANGELOG]] | Keep a Changelog — v0.0.1 → v0.2.1 派生字段层（daily_state） | #meta #changelog | active |
...
```
src/rquant/      config / logging / adapter(tushare) / storage(duckdb) / indicator / state / models
scripts/         ingest_daily.py（数据+因子+指标+派生状态一次性采集）+ status.py（人读健康报告）
tests/           unit + integration + fixtures（65 个单测）
```
...
| v0.2.1 | 2026-04-16 | **Week 3a 完成**：daily_state 派生字段层（涨停/首板/一字板/连板 等 15 列），MyTT 依赖 |
| v0.3.0 | — | Week 3b：筛选规则引擎（SQL 作为 DSL，扫 daily_state） |
