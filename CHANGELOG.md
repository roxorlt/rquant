...
- **Stage 1 资格分母与日指标链路阻断**：三策略 eligibility 完整性分母改为严格 A 股代码
  ...
  N 字 121 交易日面板使用同一口径，不改变策略触发条件。日终 ingest 在同一事务内基于
  当时及此前日线、复权因子生成 `daily_indicator`，历史日期重跑会先失效受影响代码的
  后续陈旧指标；新增默认只读的 `daily-indicator-backfill`，只有显式 `--apply` 才在
  交易保护窗口外原子重建指定日期范围。项目版本从 `0.22.0` 更新到 `0.22.1`。
