...
- **路线 A 链上的五处阻断：9 月 24 日真实数据回放暴露，修完后本地回放同一形状能走到当日 serving 代并带信号（包 AI）**：
  ...
  - **计数是心跳上的「观察值」，不让角色变成降级**：`RuntimeStepResult` 与心跳文件模型新增 `observations`（非负整数计数，
    ...
    **只在心跳文件里**，不进冻结的 serving 投影（#237）。auction_gap 发布者每轮报
    ...
    记一行被排除的代码清单。选这个而不是写进候选批次元数据，是因为候选批次文档是 `extra="forbid"` 的发布内容，加字段
...
- **盘中竞价链一个批次也发不出来：适配器缺列、空表被当成缺列、失败被早退分支洗掉（#277，本包）**：
  ...
  4. **采集窗写死在 09:26-09:30、三次重试挤在七秒里、早退分支用那份从未被写过的初始结果把
     ...
     （**09:35:00–10:05:00**），尝试次数取 **6**（`AUCTION_MATCH_DEFAULT_MAX_ATTEMPTS` 一处
     定义，设置模型默认值、生产画像与 route B 的自举派生都读它，不再各写一份字面量），
     按 `窗宽 // max_attempts` 摊在整个窗里
     （300 秒 ⇒ 09:35 / 09:40 / 09:45 / 09:50 / 09:55 / 10:00）。
     窗与次数来自 **2026-09-22 的生产探测**：同一个主 token 的 `stk_auction(20260922)` 在
     09:26:02 / 09:26:04 / 09:26:08 三次返回空，到 09:51:11 已有 6,075 行——探测只圈出了
     **09:26:08 到 09:51:11 这个区间**，首次可用的确切时刻还不知道（09-23 再探一轮更细的）。
     协调者据此裁定不猜那一刻，而是**用更多次尝试把整个区间盖住**；拿到精确时刻之后再把窗
     收紧，**定窗只有改代码常量这一条路**，顺序与时间预算见 DEPLOY.md。
     **除的是 `max_attempts` 不是 `max_attempts - 1`**：后者会让最后一次的到期时刻正好等于
     窗口右界，而闸门是「过了右界就整轮空转」，于是 2 秒轮询下有一半的相位永远拿不到最后
     那次尝试——被吃掉的恰恰是为「数据晚到」准备的那一次（独立复核 MF-1 实测）。设置模型另加一条
  ...
  `candidate.auction_gap` 的装配窗随之从 09:26-09:30 移到 **09:35-10:10**，同样是代码常量。
  ...
  缺陷；**这一段的长度 = 装配窗起点 − 09:30，按 09:35 的起点是 5 分钟**，当天数据真正出得
  更晚时（09-22 的区间说它可能晚到 09:51）会一直延到第一次采到数为止。
...
- **空闲的 notifier 每两秒一次带 fsync 的提交，以及 17:00 附近的换代重读（#271）**：
  生产上 `notifier.admin.shadow.v1` 在收盘后、外发队列为空、一条信号都没有的情况下，
  仍被采样到每秒 26 次写、约 150 KB/s。它每两秒跑一轮，这一轮里有两处写与「这一轮要不要写」
  完全无关：

  一处是页投影权威。`notification_projection_authority` 是一张只插入、内容寻址的表，
  `publish_projection_authority` 里 `WHERE generation_id = ?` 这一次查询就是「同一份内容
  发第二次是空操作」的全部机制。但 `generation_id` 是对快照**所有字段**求的哈希，其中
  `observed_at` 是 notifier 自己的时钟，`available_at` 又取自两张收据的 `published_at`——
  而生产者给两张收据盖的正是同一个 `observed_at`。于是同一份内容每一轮都算出一个新 id，
  查询永远查不中自己上一轮写下的那一行，每一轮都 INSERT、提交、并且因为
  `journal_mode=WAL` 加 `synchronous=FULL` 而真的 fsync 一次。
  现在身份拆成两个。`content_id` 只对**发布的内容**求哈希（schema 版本、来源收据、
  投影），两个时钟都不参与——所以空闲那一轮算出的 `content_id` 与上一轮相同，门控认得出
  自己上一次发布，**在只读连接上**就认得出（所以常见的那一轮连写锁都不去拿），直接返回，
  不写、不提交、不 fsync。`generation_id` 则对「`content_id` + 两个时刻」求哈希，
  表示**这一次发布**：它是表的主键，同一份内容发两次就是两行，所以「最新那一行」在构造上
  就是「最近一次发布的内容」。
  **门控比的是最新那一行，不是「有没有哪一行装着这份内容」**——投影逐字节回退到更早的形态时，
  确实有那么一行，但那一行不是正在被服务的那一行；比错了就会一直服务中间那一代。
  表新增一个可空的 `content_id` 列，旧库开库时用 `ALTER TABLE` 补上（只改 schema、不重写任何行、
  不触发任何不可变触发器）。**生产状态库里已有的行 `content_id` 为 NULL，就按旧规则读**：
  校验器认那个形态，但永远不再产出它。

  另一处是复制游标。`replicate()` 每一轮都写 `notification_replication_source`，因为
  `updated_at` 同样是这一轮的时钟——哪怕水位、末尾序号、末条信号 id 三列与库里一模一样。
  现在只有这三列中有一列真的前进了才写，`updated_at` 因此表示「游标上一次前进的时刻」。

  换代重读这一侧：notifier 的副本读档案此前只有 09:20–09:40 一段禁读时段，而 17:00 的日线
  管线读的是**主库**、与副本共用同一份页缓存（09-08、09-09 两天它卡在 `daily_state`，
  runtime role 一停就在一分钟内跑完）。notifier 的重读间隔是 15 分钟，所以一次 692 MB 的
  换代读完全可能正落在日线开头那几分钟。`ReplicaReadProfile` 现在带的是一组有序且互不重叠的
  盘中时段而不是一段，notifier 多拿 16:55–17:40：16:55 开始是因为当天最后一次副本同步在
  17:30，17:40 结束是刚好在那次同步之后——窗口之后第一次读到的就是当天最终的那一代。
  其余 role 不加，理由与 #268 里写的一样。

  心跳文件模型新增 `projection_published`（这一轮到底有没有真的写），同样只是文件字段，
  冻结的 serving 投影一个字段都没动。
...
- **题材梯队数据归属与读取韧性**：保留 KPL 股票的多题材归属；脉搏与午间战报在一次
  只读连接中加载所需数据，并在只读源不可用或测试 fake 缺少字段时降级为空结果，不阻断
  主推送链路。

### Fixed

- **全景页冷启动陈旧快照兜底**：收盘后/重启后所有活数据源不可用时，poller 从自家
  drop 落盘或 surge feed 文件恢复最后一份快照（route 标 `drop_stale`/`cloud_feed_stale`，
  `age_seconds` 按数据真实时间回算，⚠️ 陈旧标注正确触发），页面不再全空。项目版本从
  `0.28.0` 更新到 `0.28.1`。

### Added

- **爆量记录跨日搜索与分时回看**：支持按股票代码或名称搜索历史爆量记录，结果按交易日倒序排列；点选任一记录会加载该确切交易日的本地 `minute_bar` 分时，并在图上标记当日每一次有效爆量确认时点。只读副本不可用或分时数据缺失时显示明确空态；搜索结果刷新时以选择指纹避免行选中错位。项目版本从 `0.28.2` 更新到 `0.28.3`。

- **爆量记录历史日期回看**：爆量记录 tab 新增日期选择（默认今天，可回看云端留存的
  任意交易日 events），历史日期下图表标记按日期范围自然生效。

### Fixed

- **爆量 Push 跨交易日重触发与近 5 日次数**：surge-watch 启动时从只读交易日历取得
  今日及前四个交易日，只读对应事件 JSONL；过去交易日仅参与逐票次数统计，不再影响
  今日触发，同一标的跨天再次满足条件会正常推送。今日已有事件恢复到进程内去重集合，
  避免服务同日重启重复推送；手机报文逐票新增“近5日推送次数 n”（按不同交易日去重、
  包含本次）。交易日历或历史文件异常时 fail-soft，不阻断爆量监控主链。项目版本从
  `0.27.1` 更新到 `0.27.2`。
...
- **Composite VP 60 日增量滚动引擎 `src/rquant/vp/engine/composite.py`**（纯新增，
  未接入任何生产调用链、不做任何 DB 读写）：规格 §8.2 禁止每日全量重算（5400 × 60 ×
  240 ≈ 7.8 亿行），`CompositeVP` 以「单日桶直方图」为滚动窗口的最小单元，加一天与
  挤出第 61 天都是直方图相加减，不回头重扫窗口内分钟线。**bin 网格由窗口首日参考价
  锚定并全程固定**，`push_day` 收该日原始分钟集、用本窗口的网格现场分桶（复用
  `ProfileAccumulator` 的算法 U 路径，不写第二份实现）；不接受按别的参考价分好的
  单日 profile——按 bin 中值价重映射是二次量化，误差在 60 天里同向累积，POC 偏 1 个
  bin 就足以让 §3.4 的买点判定翻转。bin index 锚在价格 0 上，价格漂移只是落到更高/
  更低的 index，历史 bin 无需重分桶，「网格向两端扩展」零成本。挤出旧日用引用计数
  整桶删除而不是「减完约等于 0」：`0.1 + 0.2 - 0.1 - 0.2 == 2.78e-17` 是正数，会凭空
  撑出幽灵 bin 并拉宽稠密区间。除权重置按规格 §2.3 提供 `reset(reason)`（引擎不判断
  除权，那需要 `adj_factor`，属调用方职责），连同参考价一起清空并由下一入窗日重新
  锚定网格，状态里留 `reset_at` / `reset_reason`；`to_state()` / `from_state()` 给出
  可 JSON 落库的 Pydantic 状态，为后续 `rvp.duckdb` 持久化留接口。窗口未满 60 天照常
  出快照，由 `days_count` 说明堆了几天。测试以「随机 70 天逐日 push 的快照 == 对应
  60 天一次性重建」为金标准逐 bin 比对。引擎版本 `ENGINE_VERSION` 从 `0.1.0` 提到
  `0.2.0`（规格 §7：引擎变更需重跑历史实验）。

- **VP 计算引擎与研究沙盒脚手架 `src/rquant/vp/`**（纯新增，未接入任何生产调用链）：
  `engine/` 落地规格 §3.3 的可执行定义——Session VP 增量累加器（分钟到达即更新、
  任意时刻取快照、`from_minutes` 重放重建以配合 §3.0 的崩溃恢复要求）、Anchored VP
  （锚点选取「取最近不取最大」「选定不漂移」，分钟集/日线集两个显式入口）、
  HVN/LVN 三条件检测、`VPEngineConfig` 参数总表（§10.1 抄表，`va_pct` 硬校验锁死
  0.70）。POC 并列裁决与价值区连续扩展**直接复用** `volume_profile._poc_index` /
  `_value_area`（PR #140 的 G20 修复版），全仓只此一份实现；分桶宽度复用
  `_resolve_bin_size` 的 `bin_ratio` 分支，落桶用 `Decimal` 避免浮点掉桶。
  `sandbox/` 按 §7 建 7 个可写文件 + 权限边界 README。新增 31 条单元测试覆盖
  增量/重建一致性、算法 U 量守恒、LVN 三条否决路径与价位缩放下的形态不变性。
- **三重锁相 VP 策略工程规格 v0.3 + P0 结果**（docs/研究资产，无生产代码改动）：
  `docs/vp-strategy/` 落地完整规格书（20 个缺口补全、S1–S13 盘口级条件单子策略、
...
- **iTick 平台知识库与证伪测试**：`docs/vendor/itick/` 收录官方文档 76 页全量抓取、
  ...
  快照 5 档非逐笔、延迟 P50 1.8s/P99 3.2s。
- 附带发现：云端 `daily_bar` 存在 2024-09~2025-04-20 稀疏空洞（迁云历史回补止于
  2024-08-30 所致），修复待授权后回补。

### Changed

- 项目版本从 0.27.2 更新到 0.28.0
...
- **策略分钟回补 manifest 可审计终止**：新增 `abandoned` 终态和
  ...
  旧版 SQLite 尚无终止列时，dry-run 通过兼容投影保持严格只读；只有已确认的 `--apply`
  才迁移状态表并写入终态。

- **Stage 1 按策略独立验收**：新增只读 `stage1-acceptance` 计划和
  `scripts/run-stage1-strategy-acceptance.sh`。验收身份精确绑定单一策略、manifest、日期范围
  和干净代码提交；N 字与科创/创业放量不再读取或等待集合竞价 manifest。脚本保存修复、
  快照、审计和正式回放证据，所有写入均在交易保护窗口外按顺序执行，首错即停并恢复原
  timers；相同 manifest 重跑复用已固化的 snapshot as-of。预算改按冻结的股票交易日窗口
  估算现有分钟扫描行数，避免把“无需继续下载”误报为“快照无需扫描”。项目版本从
  `0.25.4` 更新到 `0.26.0`。
...
- **30 分钟脉搏 Push 手机端字段堆叠**：正文改为“市场温度 / 新晋涨停 / 题材热度 /
  ...
  增量口径、数量上限、数据源标注、通知路由和午间战报保持不变。项目版本从 `0.26.8`
  更新到 `0.26.9`。

- **爆量 Push 手机端字段堆叠**：每只新确认标的改为独立 Markdown 分组，代码、名称与
  临近涨停状态置于标题，题材、涨幅/涨停空间、累计比/累计额、分钟方向分别换行展示；
  判定口径、每日去重、每批上限、折叠规则与 PushDeer-only 路由保持不变。项目版本从
  `0.26.7` 更新到 `0.26.8`。
...
- **成长板回补覆盖规划反复扫描原始分钟表**：运营库分钟完整性检查改为跨日期的 8192
  ...
  2026-07-08 的 66 个交易日生产只读隔离基准可完整结束：22,756 条资格记录、359 个
  下载任务、2,209 个待补 session，总耗时 28 分 31 秒，峰值约 1.80 GiB，未触发 OOM。
  项目版本从 `0.26.4` 更新到 `0.26.5`。

- **科创/创业 Stage 1 回补规划耗尽生产内存**：成长板开盘结构分类改为按目标日稳定
  分批，资格解析与输入覆盖复用同一份结构事实，不再执行两次全范围分类；
  `backfill-plan` 的独立 DuckDB 连接限制为默认 2048 MB、2 线程，并使用命令级临时
  spill 目录，异常退出后自动清理。候选解析在前一交易日均线完整时只读取当日收盘，
  仅为缺少任一均线的代码回退读取 60 个交易日日线。修复前单进程约 5.9 GiB 后被内核
  OOM；受限内存的生产只读隔离基准在手动终止前运行 43 分 32 秒，峰值约 1.81 GiB，
  未再触发 OOM。该基准未完成，完整 planner 耗时与 Stage 1 manifest 仍需按新快速路径
  另行验收。项目版本从 `0.26.3` 更新到 `0.26.4`。

- **Stage 1 分钟审计在生产范围内耗尽 8 GiB 内存**：完整会话规则不再为 55,363 个
  股票交易日会话同时聚合 241 个时间字符串，改为主键保证下的计数式网格检查，并仅为
  最多 20 个异常样本读取分钟明细；跨数据源重叠规则按实际交易日分区扫描后累计全范围
  总数和稳定样本。2026-04-01 至 2026-07-06 生产只读实测由约 5.9 GiB、被内核 OOM
  终止，降为峰值约 1.41 GiB、约 1 至 2 分钟完成，审计 P0=0。项目版本从 `0.26.2` 更新到
  `0.26.3`。
...
- **爆量粗筛累计额被上一交易日收盘 K 线冻结**：`surge-watch` 于 09:25 启动时，
  ...
  时间回退。2026-07-21 生产证据显示创业/科创 2,010 只股票的 241 点累计额整日零变化，
  粗筛分子冻结而进度曲线分母持续增长，因此 6 个候选全部出现在 09:31-09:34，之后不可能
  再有新候选。真实盘中累计器现绑定当日交易日期，非当日分钟行既不累计也不更新分钟锚点；
  保留同分钟去重、乱序保护和同日重启 seed。项目版本从 `0.25.3` 更新到 `0.25.4`。

- **爆量通知把放量下跌误当成主动买盘**：`surge-watch` 粗筛阶段虽要求红盘，但候选排队
  后的最终确认没有重新检查价格，导致转跌甚至跌破昨收仍能推送；全市场 `rt_min` 最新根
  略滞后时也可能与单票精确分钟线方向不一致。口径升级为 v4：保留 v3 同刻累计成交额
  核心，在确认分钟用 `rt_min_daily` 仅取当时及之前数据，重新核对红盘、1 分钟上涨及
  tick-rule 近似外盘大于内盘；精确分钟尚未覆盖当前格时暂缓而不是误报。事件与推送新增
  1 分钟涨幅、近似外/内比和价格来源。科创/创业放量回放中原本把“外盘>内盘”实现成
  `inner/outer > 1`，现修正为 `< 1`，因子评分同步奖励外盘占优；2026-07-03 受影响的
  内外盘消融结论已标注失效、等待按修正口径重跑。项目版本从 `0.25.2` 更新到 `0.25.3`。

- **备份服务在主库增大后固定超时**：生产 DuckDB 增至约 5 GB 后，已验证的副本复制、压缩
  和完整性检查实测需要 194 秒，原 `TimeoutStartSec=120` 会在即将完成时终止任务，使
  `latest.duckdb.gz` 长时间停留在旧代际。备份服务启动预算调整为 10 分钟，保留脚本自身
  的原子发布和完整性校验。

- **策略分钟回补静态 ETA 严重低估且网络请求完全串行**：生产实测
  `growth_board_surge` 的 5,493 个任务耗时约 10 小时，而原静态模型只估 0.59 小时；
  `auction_gap` 的 21,726 个任务因此不能按 2.3 小时估算直接长跑。`backfill-run`
  现在默认使用 8 个独立 Tushare worker 并发拉取，全部 DuckDB 检查、upsert 和复验仍
  经过同一把进程锁串行，SQLite claim token、lease fencing、失败隔离和断点恢复语义不变。
  ETA 从 SQLite 最近 128 个同源、同频率、同响应上限且真实发过请求的成功任务提取 P75，
  同时比较秒/任务和秒/预计行，按 worker 数折算后保留 25% 并发损耗与 2 倍保护余量。
  同类成功任务不足 32 个时同时采用成长板生产实测冷启动下限
  （10.416 秒/任务、651 行/秒），避免新 manifest 继续被静态模型严重低估。
  尾部估算使用实际有效 worker 数，并保留单任务 P75 与 API 限频墙钟下界。
  新增 `--workers` 和 `--max-runtime-minutes`；分段运行的实际截止永远不晚于下一交易保护
  窗口前 10 分钟；截止后丢弃尚未写入的数据并无损释放 claim，5 分钟后由父进程硬终止，
  再保留 5 分钟恢复 timers；普通 claim 与最终 recovery-only claim 均保持原尝试次数。
  保护窗口内仍拒绝启动。`scripts/rollout-v0.25.2-stage1.sh` 可显式恢复已冻结的长
  manifest，逐个验证原 timers 恢复 active；工作日 09:05 后拒绝启动恢复，完整链只在
  15:10 后启动，且备份、修复、快照等全部长子进程受下一 09:10 的统一硬截止监管。
  项目版本从 `0.25.1` 更新到 `0.25.2`。

- **Tushare 空停牌时段被误判为未知会话**：`suspend_d` 的停牌 `S` 行在
  `suspend_timing` 为空时按官方样例语义规范化为全天停牌；显式日内时段仍为部分停牌，
  复牌 `R` 的空时段仍保持未知。修复成长板资格面板把已由完整停牌快照证明的非交易股票
  当作日线/指标缺口，进而让 99% PIT 输入门错误失败的问题。全天停牌豁免统一排除同日
  日线、非零分钟成交、复牌或部分停牌冲突；零成交分钟占位行不构成冲突。历史
  `suspension-backfill --full-refresh` 先完成全部源请求，再在一个 DuckDB 事务中发布
  整段快照，任一日期失败会回滚事件与覆盖表；新增 `--dry-run`，在不构造数据源适配器、
  不联网和不写库的前提下展示权威开市日与精确刷新范围。成长板长区间回放改为一次批量
  计算全部目标日的开盘结构，避免逐日重复扫描停牌和分钟证据。新增
  `scripts/rollout-v0.25.1-stage1.sh`，固定精确 tag/SHA、15:10 含端点保护、主库备份、
  原 timer 状态恢复，以及三策略 manifest、分钟修复、snapshot、审计和正式回放证据链。
  旧 manifest 保持冻结，三套正式策略证据必须按新代码提交重新生成。项目版本从
  `0.25.0` 更新到 `0.25.1`。
...
- **策略分钟回补会规划尚未发生的退出交易日**：`backfill-plan` 不再要求人工猜测候选
  ...
  N 字策略显式记录次日入场偏移，科创/创业放量与集合竞价保持当日入场；`15:10`
  仍属于保护窗口，只有其后才计入当天。显式请求晚于可观测上限时失败关闭，底层规划器
  也会拒绝任何晚于 manifest `as_of_time` 最近已收盘交易日的真实需求；`backfill-run`
  在领取任务前会再次用权威交易日历核验持久化 manifest 及任务范围，并严格比对内嵌
  plan 与 SQLite 实际可领取任务的 ID、完整 payload 和存储内容哈希，旧错误计划或
  状态分叉不能绕过新规则重跑。旧 manifest 缺少新字段时保持原身份哈希，不破坏历史
  审计证据。项目版本从 `0.23.2` 更新到 `0.23.3`。

- **确定性停牌缺口误阻断 N 字策略历史资格面板**：N 字 121 交易日输入完整性继续以
  全量 A 股为分母；当且仅当 Tushare 停牌快照覆盖完整、同一股票当日存在停牌 `S` 且
  不存在复牌 `R`、同时权威日线确实不存在时，将该股票日计为确定性非交易输入，不再要求
  不可能存在的日线、派生状态和流通市值。覆盖不完整、同日复牌或残留日线仍失败关闭，
  不改变策略候选条件、分钟数据缺失语义和 99% 完整性门槛。项目版本从 `0.23.1`
  更新到 `0.23.2`。

- **无成交竞价观测被误判为历史数据缺失**：`stk_auction` 会为部分股票明确返回
  `price=NULL, vol=0, amount=0`，表示开盘集合竞价没有撮合成交；历史修复质量门此前只把
  正价格、正成交量计为有效，导致正常无成交比例略高的交易日错误跌破 98% 覆盖门。修复后
  将有限正价格/成交量/成交额的成交行与这一精确零成交组合共同计为结构完整，98% 双向门槛
  保持不变；空价格配正成交量、非正值、无穷值等半截行一律标为畸形并阻断整批发布。质量
  计划契约升级到 `research-auction-history-repair/v2`，显式绑定成交、明确无成交和畸形
  代码数；集合竞价策略仍只消费正价格、正成交量的真实成交行。项目版本从 `0.23.0`
  更新到 `0.23.1`。

- **历史日线修复会破坏状态尾段，短上市/停牌股票又阻断科创创业资格门**：
  `market-daily-backfill` 在逐日原子写入市场事实时明确失效受影响的 `daily_state` 与
  `daily_indicator` 尾段，避免新日线继续配用旧派生结论；最终按修复日前最后一个权威状态为
  种子，分批准备完整状态尾段后在单个 DuckDB 事务内一次替换。派生、暂存或最终发布任一步
  失败都保持整个尾段缺失并失败关闭，不会留下部分新状态；缺少直接前序状态时在写市场事实
  前阻断。科创/创业开盘面板继续保留全部股票在完整性分母中，但对权威
  上市日证明不足 60 个可交易会话、或 T-1 有完整 Tushare 全日停牌证据的确定性非候选，
  允许计为结构性完整，并用同一分类强制从真实候选中排除；日历缺口不能取得结构性豁免，
  缺上市事实不再用首条本地日线猜测，混合停复牌、停牌与残留 K 线/指标冲突、部分停牌和
  恰好 60 个会话仍失败关闭。
  项目版本从 `0.22.4` 更新到 `0.22.5`。

- **空交易日阻断 N 字策略回补计划**：筛选加载器在目标日没有可见宽表时会返回零列空表；
  `screen()` 现在先把任何空输入规范化为固定结果 schema，再应用候选白名单，避免
  `backfill-plan --strategy n_shape` 在无候选交易日访问不存在的 `ts_code` 列而中断。
  非空但缺关键列的数据仍保持 fail closed。项目版本从 `0.22.3` 更新到 `0.22.4`。

- **平价窗口导致日指标回补中断**：KDJ 在窗口最高价等于最低价（一字价、停牌价等）
  时不再用 `pd.NA` 把零价差分母提升为 object dtype，改为保留 `float64` 的 NaN，并按
  既有公式沿用前一 K/D 值；修复 Pandas 3.0 下转换 `NAType` 时抛错、全市场日指标无法
  重建的问题。项目版本从 `0.22.2` 更新到 `0.22.3`。

- **历史复权因子稀疏导致日指标回补假成功**：统一数据集回补注册表新增只写
  `adj_factor` 的按交易日幂等路径，可独立预演和修复全市场复权因子而不删除
  `daily_state` 等旁表。日指标历史回补改用严格 A 股日线键作为分母；派生覆盖率低于
  99% 时在打开主库 writer 前明确失败并保留现有指标，不再出现预计约 40 万行却只写入
  数十行仍返回成功。项目版本从 `0.22.1` 更新到 `0.22.2`。

- **Stage 1 资格分母与日指标链路阻断**：三策略 eligibility 完整性分母改为严格 A 股代码
  范围，不再把 `200xxx.SZ`、`900xxx.SH` B 股计入缺失分母；竞价/科创创业开盘面板与
  N 字 121 交易日面板使用同一口径，不改变策略触发条件。日终 ingest 先从权威主库只读
  快照分批计算当日 `daily_indicator`，关闭读连接后再在日终事务内原子写入；历史日期
  重跑会先失效受影响代码的后续陈旧指标；新增默认只读的 `daily-indicator-backfill`，只有显式 `--apply` 才在
  交易保护窗口外使用隔离副本计算，并在短写事务内核对主库源事实指纹后原子重建指定日期
  范围。项目版本从 `0.22.0` 更新到 `0.22.1`。
...
- **研究数据湖导出契约**：新增 `research-export`，从本地只读副本按交易日（分钟数据额外按
  ...
  逻辑内容哈希、物理文件哈希、代码提交和来源。独立 `research.duckdb` 只用短事务记录当前分区、
  覆盖度和每次导入/替换证据；重复导出幂等，dry-run 不创建目录或元数据库，为后续存量研究
  数据迁云提供可审计底座。项目版本从 `0.17.3` 更新到 `0.18.0`。
