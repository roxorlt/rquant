...
- **科创/创业 Stage 1 回补规划耗尽生产内存**：成长板开盘结构分类改为按目标日稳定
  ...
  仅为缺少任一均线的代码回退读取 60 个交易日日线。运营库分钟覆盖改为跨日期的 8192
  session 有界批次；`combined` 模式只把运营库缺口交给研究湖，同时仍保留完整 artifact
  provenance。修复前单进程约 5.9 GiB 后被内核 OOM；2026-04-01 至 2026-07-08 的 66
  个交易日生产只读隔离基准现可完整结束：22,756 条资格记录、359 个下载任务、2,209 个
  待补 session，总耗时 28 分 31 秒，峰值约 1.80 GiB，未使用 swap 或触发 OOM。项目版本
  从 `0.26.3` 更新到 `0.26.4`。
