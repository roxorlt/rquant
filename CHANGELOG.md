...
- **空闲的 notifier 每两秒一次带 fsync 的提交，以及 17:00 附近的换代重读（#271）**：
  ...
  现在 id 只对内容求哈希：`observed_at` 与 `available_at` 都不参与。内容没变就查得中自己
  那一行，**在只读连接上**查得中——所以常见的那一轮连写锁都不去拿——直接返回，不写、不提交、
  不 fsync。两个时间字段仍然是字段也仍然是列，含义不变，只是那一行记的从「最近一次发布」
  变成了「第一次发布这份内容」；表本身禁止 UPDATE，所以每份内容只写一次。
  **生产状态库里已有的行是按旧规则写的，按旧规则读**：校验器认那个形态，但永远不再产出它。
