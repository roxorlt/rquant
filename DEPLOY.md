...
2026-09-22 又用同一个主 token 只读探测一轮，结果是一个**区间**而不是一个时刻：

| 时刻 | `stk_auction(20260922)` |
|---|---|
| 09:26:02 / 09:26:04 / 09:26:08 | **空**（0 行） |
| 09:51:11 | **非空**，6,075 行 |
...
**四个窗口常量必须一起改**：两对窗口的一致性（装配窗与采集窗同时开始、在它之后关闭）由
...
的**行数**（`SELECT count(*) FROM notification_projection_authority`）与**文件大小**。
理由：本包的门控每一轮要对这张表做一次带排序的扫描，**耗时按行数走**——复审在合成库上
实测 5000 行约 29 ms、20000 行约 91–107 ms、80000 行约 397–657 ms（这是把 payload 拖进
排序器的第一版；发出去的这一版只选两列，同样规模是 3.7 ms / 17.9 ms）。生产这张表只插不删，
所以行数只会涨；记下装机当天的起点，下一次有人怀疑门控自己变贵时才有对照。

**要修的现象**（09-16 只读代码确认，v0.33.12）：

1. 收盘后、外发队列为空、一条信号都没有的情况下，`notifier.admin.shadow.v1` 仍被采样到
   **每秒 26 次写、约 150 KB/s**。它每两秒一轮，一轮里有两处写与「这一轮有没有事情做」无关：
   页投影权威每轮 INSERT 一行（`generation_id` 把 `observed_at` 和 `available_at` 两个
   本轮时钟都算进了哈希，所以同一份内容每轮算出新 id，去重查询永远查不中自己），
   复制游标每轮重写一次（`updated_at` 同样是本轮时钟）。两处都是 WAL 加
   `synchronous=FULL` 上的 `BEGIN IMMEDIATE … COMMIT`，每一次都是真的 fsync。
   **口径要准确**：生产 manifest 现在给 notifier 的是 `paused: true`，暂停分支不调
   `replicate()`，所以**今天生产上真正在发生的是页投影这一处**；复制游标那一处在
   notifier 取消暂停的那一刻就会开始，本包一并修掉，现在不会有人踩到它。
2. notifier 的副本禁读时段只有 09:20–09:40，**17:00 附近没有**。它的重读间隔是 15 分钟，
   一次 692 MB 的换代读可以正落在日线管线开头、页缓存最冷的那几分钟。

**装上之后当场应该看到什么**：

- **收盘后（比如 16:00–16:50 之间）对 `notification_state.sqlite3` 做两次 `stat`，间隔
  十分钟，`st_mtime` 应当完全没动**，前提是这段时间没有新信号也没有新的一代副本。
  这是本包最直接的验收：文件不动 = 没有提交 = 没有 fsync。装之前同样两次 `stat` 必然在动。
- notifier 的心跳文件里出现新字段 **`projection_published`**。收盘后它应当**绝大多数轮
  是 `false`**，只在副本换代且内容真的变了的那一轮是 `true`。
  **恒为 `true` 就是本包没装上**，要当故障查。
- 16:55–17:40 之间 notifier 不开库，心跳上 `replica_opened=false`、
  `replica_skipped_by_floor=true`（与 09:20–09:40 同一机制、同一时钟）。
- 17:40 之后第一次 `replica_opened=true`，读到的是当天 17:30 那次同步落下的最终一代。

**当天 17:00 的观察按这一条走**：**15:15 装机，之后不停不起，17:00 的日线管线照常跑**，
看 `daily_state` 阶段有没有再卡。**本包不宣称能单独解决 17:00 停滞**——#271 里那两处是
已经确认的缺陷，17:00 停滞的完整机制仍待当天现场采样。若装上之后 17:00 仍然卡，那一次采样
就少了「notifier 每两秒 fsync」和「换代读撞进来」这两个变量，正是这次观察的用处。

**一个取舍：收盘那 45 分钟页面会更旧。** 16:55–17:40 禁读，加上副本本身最多旧 5 分钟，
**这段时间 `minute_coverage.max_time` 最坏落后约 50 分钟**。收盘后数据本来就不再变，
代价基本落在「日线刚跑完、页面还没跟上」这半小时里；如果 owner 要在 17:00–17:40 之间
看当天刚落库的日线，请提前说，窗口可以缩到 16:55–17:15。
...
**（2026-09-17 更新：第十个窗口没有发生，这四包一直没装。它们现在跟包 V、包 W 一起走
第十一个窗口，装的是那一刻最高的那个 tag——见本文件最上面那一条。本条下面写的现象、
验收判据和回滚步骤全部照样有效，只是装机的那一个 tag 换了。）**
...
**状态**：**尚未安装**。本条是安装前必读，不是部署记录。
**装机口径（协调者 2026-09-12 定）**：本包与包 S 在**第十个窗口**一起装，装的是**一个 tag
`v0.33.11`**——包 S 合入后打 `v0.33.10`，本包合入后打 `v0.33.11`，部署器 target 取后者，
包 R 的 v0.33.9 也在这一批里一起上线；**时机由协调者决定，不早于 2026-09-14（周一）跨交易日
观察结束**。
...
**R-31 在 `v0.33.11` 装上之后作废**：它存在的唯一理由是 router 撞上这个形状会退 1 并推送一条，
而这一包把那个形状改判为等待。装完 `v0.33.11` 之后滚动重启不必再讲先后顺序，也不必再去数
`runner.sqlite3-shm`——请把 R-31 标成已退役，不要再按它操作（照它做不会坏，只是多一道白费的等待）。
反过来说，**`v0.33.11` 装上之前 R-31 仍然有效**，回滚到 `v0.33.10` 或更早也要把它重新当真。
