...
rQuant 投研平台的网页（React 19 + TypeScript + Vite，入口 `/app/`）。本文件与 web/ 下另一份代理约定内容相同，改一个就同步另一个。
仓库级规则（分支、提交、合并、部署）见根目录 `CLAUDE.md`；计划与排期见新前端计划 v2。

## 产品目标与迁移

CC 可点击原型是最终页面与交互目标，不是只做相似外观的示意图；原型里的功能和按钮最终都须接通真实能力。线上旧 Streamlit 页面资源负担高、体验不符合 owner 目标，不再为旧页面增加功能、打磨界面或扩容；必要的故障与安全维护除外。

按功能逐项实现、验证并迁移对应入口，再停用被替代的 Streamlit 单元。最终补齐差距表全部「部分」「缺」，以及 M4、M12 虽标「已有」但仍列出的未完项；M11 实盘交易排除。未验证完成或未获生产切流、停服授权的功能，不能宣布已迁移。长期决策与验收条件见 `docs/plans/2026-09-26-react-platform-decision.md`。页面设计遵守下方「界面与文案原则」。
