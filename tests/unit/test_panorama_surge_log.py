...
def test_dedup_keeps_earliest_confirmed_at(tmp_path: Path) -> None:
    ...
    confirmed = [
        SurgeConfirmed(
            ts_code="300001.SZ", name="特锐德", theme="充电桩", confirmed_at="09:45",
            pct_chg=6.1, cum_amount=3.2e8, rel_cum=3.4, room_to_limit_pct=4.2,
            status="confirmed",
        ),
        SurgeConfirmed(
        ...
        SurgeConfirmed(
            ts_code="300750.SZ", name="宁德时代", theme="锂电池", confirmed_at="10:15",
            pct_chg=9.9, cum_amount=8.0e8, rel_cum=5.0, room_to_limit_pct=0.3,
            status="unbuyable",
        ),
    ]
