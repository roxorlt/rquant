...
def test_normative_baseline_pair_and_candidate_repository_identity() -> None:
    ...
    # main; this one moves it from PR #285's to PR #288's. It is the merge base of the
    # endpoints an R07 run states, not something rediscovered from a ref.
    assert BASELINE_COMMIT_SHA == "911f24125e05bdaea652744ccac5edd43afb6d75"
    assert BASELINE_TREE_SHA == "5da8052aa6dccf214c87231bd0c526daf648bb3f"
...
def test_candidate_gate_requires_the_historical_baseline_to_remain_an_ancestor(
    ...
    including this topic's ``911f241`` - does have ``45d0b57c`` behind it, so on this
...
def test_production_category_is_reserved_for_declaration_scanned_sources() -> None:
    ...
    # Re-aimed twice, and the two refreezes since are why naming paths here was wrong.
    # Release B dropped scripts/r07_deploy_gate.py from this spot: it was in the previous
    # baseline's diff only because that release created it, so a refrozen baseline left the
    ...
    # rollout store and the installer acknowledgement needed no new job, the refreeze to
    # a0bbb4c put it back once more because the credential shape topic marked fifteen cases
    # ``linux_exact`` that no shard collects and gave them a rooted container job of their own,
    ...
    # the refreeze to 64ea140 left it out for that same plainest reason - the sandbox path
    # gap topic containing no CI change at all - the refreeze to 7b98e80 left it out once
    # more because the generation handover topic added no job, the refreeze to 3cdfa22 left
    # it out because the seventh-window gap topic touched no workflow either, the refreeze
    # to 99ef54a left it out because the read-side replica topic touched no workflow either,
    ...
    # it out because the router runner peer-wait topic touched no workflow either, the
    # refreeze to 4e76850 left it out because the open-time I/O interference topic touched no
    # workflow either, the refreeze to 00d74c6 left it out because the notifier idle-fsync
    # topic touched no workflow either, the refreeze to 79bccd8 left it out because the
    ...
    # but it does touch .github/workflows/ci.yml, for a third reason neither of the two
    # earlier ones predicts: its e2e worlds landed in shard 1 and the 3.12 lane of that shard
    # was cancelled at the job's 100-minute limit twice with nothing failed, so the limit
    # moved to 120. The matrix is still five shards and no job was added; what changed is a
    ...
    # The refreeze to 611d464 left it out again: the signal-chain / notifier-shadow /
    ...
    # And this refreeze to 911f241 leaves it out as well: the auction-match finite-row topic
    # adds no module under src/rquant/ and changes five, adds no test file and touches six
    # existing test modules, changes no script, marks nothing ``linux_exact``, adds no job and
    # does not touch .github/workflows/ci.yml. It regenerates the shard manifest because the
    # test count changed (14758 -> 14763); the matrix is unchanged at five shards. Of the nine
    # source_file_snapshots modules it changes one, runtime_service_builtin.py (the capture
    # window constants), so that snapshot and the one boundary probe living in it,
    ...
    # in. So this refreeze is the first without a flip since then, and a named path would
    # have flipped truth value eleven times in thirty refreezes: naming one
    ...
    # below against whatever tooling this diff does contain, and the category rule for those
    # two exact paths is pinned directly, independently of any diff, by
