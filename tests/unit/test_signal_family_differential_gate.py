...
def test_normative_baseline_pair_and_candidate_repository_identity() -> None:
    ...
    # main; this one moves it from PR #229's to PR #234's. It is the merge base of the
    # endpoints an R07 run states, not something rediscovered from a ref.
    assert BASELINE_COMMIT_SHA == "aca0dd8e8d72e49390b55ac16ce85a4596de0731"
    assert BASELINE_TREE_SHA == "a0a86c5f3c1ee45fe8c46092f2b6304434afd4c1"
...
def test_candidate_gate_requires_the_historical_baseline_to_remain_an_ancestor(
    ...
    including this topic's ``aca0dd8`` - does have ``45d0b57c`` behind it, so on this
...
def test_production_category_is_reserved_for_declaration_scanned_sources() -> None:
    ...
    # and this refreeze to aca0dd8 takes it out again: the idle live chain topic marks nothing
    # ``linux_exact``, adds no job, and leaves that workflow untouched.
    # A named path would have flipped truth value
    # nine times in twelve refreezes: naming one
