...
def test_research_ingest_service_uses_daily_default_and_failure_relay() -> None:
    content = (SYSTEMD / "rquant-research-ingest.service").read_text(
        encoding="utf-8"
    )

    assert "After=network-online.target rquant-daily.service" in content
    assert "rquant-replica-sync.service" in content
    ...
    assert (
        "ExecStart=/home/lighthouse/rquant/scripts/run-research-ingest-daily.sh"
        in content
    )
...
def test_research_ingest_timer_runs_after_daily_without_cross_day_catchup() -> None:
    content = (SYSTEMD / "rquant-research-ingest.timer").read_text(
        encoding="utf-8"
    )
