...
The order is the chain's own: the publisher at 09:37, inside the window #277 moved it to,
then the two sources at 09:40, in the morning phase, reading what it published.
"""
...
#: 09:37 Asia/Shanghai: inside the publisher's assembly window, which #277 moved to
#: 09:35-10:10 because Tushare's `stk_auction` has no rows for the session at 09:26 (the
#: 2026-09-22 probe bracketed first availability between 09:26:08, still empty, and
#: 09:51:11, 6,075 rows). Twelve minutes after `rquant-monitor` took the main database's
#: write lock, and just after the auction-match source captured the batch this publisher
#: consumes.
PUBLISH_AT = datetime.combine(TRADE_DATE, time(9, 37), tzinfo=_SHANGHAI).astimezone(UTC)
#: 09:40, the morning phase and after the publisher: the two sources load a candidate
#: universe only from 09:30 (`may_fetch_market_minute`), and a snapshot captured at 09:37
#: is not visible to a reader whose `as_of` is earlier than it.
CONSUME_AT = datetime.combine(TRADE_DATE, time(9, 40), tzinfo=_SHANGHAI).astimezone(UTC)
...
AUCTION_AVAILABLE_AT = datetime.combine(
    TRADE_DATE, time(9, 35, 5), tzinfo=_SHANGHAI
).astimezone(UTC)
...
def test_the_auction_gap_publisher_publishes_inside_its_window(
    ...
    """09:37, inside the moved window: it reads the replica and publishes."""
    ...
    #: not "it did not crash": the step returns an empty result outside 09:35-10:10 and a
...
def _publish_every_candidate_authority(session_world: ReplicaWorld) -> None:
    """The three candidate publishers, at 09:37, in the order the profile lists them.
    ...
    time (08:45) has passed by 09:37, so one pass each is still the whole setup.
    """
