...
class ExecutionCostMathMatch(RuntimeContractModel):
    """Non-authoritative equality diagnostic for already-resolved cost math."""

    matches: bool
    reason: str = Field(min_length=1)


def _math_mismatch(reason: str) -> ExecutionCostMathMatch:
    return ExecutionCostMathMatch(matches=False, reason=reason)


def compare_execution_cost_math(
    research: ExecutionCostBindingEvidence,
    persisted_spec: ExecutionCostSpec,
    persisted_calculations: tuple[ExecutionCostCalculation, ...],
) -> ExecutionCostMathMatch:
    """Compare math only; callers must not treat a match as paper-ledger authority."""

    if research.rate_only:
        return _math_mismatch("RATE_ONLY_RESEARCH_COST")
    if research.provenance_state == "V3_UNBOUND":
        return _math_mismatch("UNBOUND_RESEARCH_COST")
    if research.provenance_state == "LEGACY_UNKNOWN":
        return _math_mismatch("LEGACY_UNKNOWN_COST_PROVENANCE")
    if research.execution_cost_spec is None:
        return _math_mismatch("MISSING_COST_SPEC")
    research_spec = research.execution_cost_spec
    paper_spec = ExecutionCostSpec.model_validate(persisted_spec)
    if not research_spec.is_alignment_eligible or not paper_spec.is_alignment_eligible:
        return _math_mismatch("LEGACY_COST_SCHEMA")
    if research_spec.cost_spec_id != paper_spec.cost_spec_id:
        return _math_mismatch("COST_SPEC_ID_MISMATCH")
    if research_spec.cost_engine_version != paper_spec.cost_engine_version:
        return _math_mismatch("COST_ENGINE_VERSION_MISMATCH")
    ...
    if research_spec.slippage.owner != paper_spec.slippage.owner:
        return _math_mismatch("SLIPPAGE_OWNER_MISMATCH")
    if (
        ...
        return _math_mismatch("SLIPPAGE_OR_PRICE_ROUNDING_MISMATCH")
    ...
    if (
        ...
        return _math_mismatch("FEE_BASIS_OR_MONEY_ROUNDING_MISMATCH")
    if len(research.calculations) != len(persisted_calculations):
        return _math_mismatch("FILL_TOPOLOGY_MISMATCH")
    for research_calculation, paper_calculation in zip(
        ...
        if research_calculation.order_input != paper_calculation.order_input:
            return _math_mismatch("NOTIONAL_OR_FILL_TOPOLOGY_MISMATCH")
        if research_calculation.instrument_context != paper_calculation.instrument_context:
            return _math_mismatch("INSTRUMENT_CONTEXT_MISMATCH")
        if research_calculation.selected_rule_ids != paper_calculation.selected_rule_ids:
            return _math_mismatch("SELECTED_RULE_MISMATCH")
        if research_calculation != paper_calculation:
            return _math_mismatch("RESOLVED_CALCULATION_MISMATCH")
    return ExecutionCostMathMatch(matches=True, reason="EXACT_MATH_MATCH")
